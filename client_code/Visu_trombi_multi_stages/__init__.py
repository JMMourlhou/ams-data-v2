from ._anvil_designer import Visu_trombi_multi_stagesTemplate
from anvil import *
...
class Visu_trombi_multi_stages(Visu_trombi_multi_stagesTemplate):
    def __init__(self, rows, intitule, pdf_mode=False, **properties):
        ...
        for b in (
            self.button_annuler,
            self.button_annuler2,
            self.button_trombi,
            self.button_trombi_pdf,
        ):
        ...
        self.label_titre.text = (
            f"Trombi stagiaires, {cod} du {date}.   (Stage num {num_stage})"
        )
