from ._anvil_designer import Stage_form_com_results_stagiaire_adminTemplate
...
class Stage_form_com_results_stagiaire_admin(Stage_form_com_results_stagiaire_adminTemplate):
    ...
    def drop_down_stagiaires_change(self, **event_args):
        ...
        max_points_ferm = (
            nb_formulaires * 5
        )  # max de points possibles pour une question
        #  ================================================================  Affichage résultats
        ...
        if (nb_questions_ferm > 0):  # Check du nb de questions fermées à afficher et affectation des questions
            ...
            )  # Je prends le 1er elmt de la liste (la question), le 2eme: si question 'obligatoire / facultative'
        if nb_questions_ferm > 1:
            self.column_panel_2.visible = True
            self.label_2.text = (
                dico_q_ferm["2"][0] + "        " + str(pourcent_q2) + " %"
            )
        if nb_questions_ferm > 2:
            self.column_panel_3.visible = True
            self.label_3.text = (
                dico_q_ferm["3"][0] + "        " + str(pourcent_q3) + " %"
            )
        if nb_questions_ferm > 3:
            self.column_panel_4.visible = True
            self.label_4.text = (
                dico_q_ferm["4"][0] + "        " + str(pourcent_q4) + " %"
            )
        if nb_questions_ferm > 4:
            self.column_panel_5.visible = True
            self.label_5.text = (
                dico_q_ferm["5"][0] + "        " + str(pourcent_q5) + " %"
            )
        if nb_questions_ferm > 5:
            self.column_panel_6.visible = True
            self.label_6.text = (
                dico_q_ferm["6"][0] + "        " + str(pourcent_q6) + " %"
            )
        if nb_questions_ferm > 6:
            self.column_panel_7.visible = True
            self.label_7.text = (
                dico_q_ferm["7"][0] + "        " + str(pourcent_q7) + " %"
            )
        if nb_questions_ferm > 7:
            self.column_panel_8.visible = True
            self.label_8.text = (
                dico_q_ferm["8"][0] + "        " + str(pourcent_q8) + " %"
            )
        if nb_questions_ferm > 8:
            self.column_panel_9.visible = True
            self.label_9.text = (
                dico_q_ferm["9"][0] + "        " + str(pourcent_q9) + " %"
            )
        if nb_questions_ferm > 9:
            self.column_panel_10.visible = True
            self.label_10.text = (
                dico_q_ferm["10"][0] + "        " + str(pourcent_q10) + " %"
            )
