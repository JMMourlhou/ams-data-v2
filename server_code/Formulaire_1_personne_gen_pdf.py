...
def enquete_1_personne_pdf_gen(formulaire_row, stage_row, role="T", type="suivi"):
    """
    Produit un PDF 'Résultat Formulaires de suivi de stages' pour 1 stagiaire donné.
    ...
    if role=="S":   # ===================================================== Formulair d'1  STAGIAIRE
        sous_titre = f"Stage {stage_code} ({stage_num}) du {date_txt}"
        if type=="suivi":
            titre_haut = "Formulaire de suivi / Stagiaire"
        else:
            titre_haut = "Formulaire de FIN de stage / Stagiaire"
    else:     # ===================================================== Formulaires d'1 Tuteur
        sous_titre = f"Stage {stage_code} ({stage_num})"
        if type == "suivi":
            titre_haut = "Formulaire de suivi / Tuteur"
        else:
            titre_haut = "Formulaire de FIN de stage / Tuteur"
        
    # Construire le bloc stagiaire à partir du formulaire
    people_blocks = []
    
    # Lecture nom/prénom de l'auteur du formulaire
    if type=="suivi": 
        user_row = app_tables.users.get(email=formulaire_row['user_email'])
        nom = user_row['nom']
        prenom = user_row['prenom']
    else:
        try:
            user_row = app_tables.users.get(email=formulaire_row['user_email']['email'])
            nom = user_row['nom']
            prenom = user_row['prenom']
    ...
    rep_ouv = formulaire_row.get('rep_dico_rep_ouv') or {}
    rep_ferm = formulaire_row.get('rep_dico_rep_ferm') or {}
    ...
    return anvil.server.call("render_pdf", html_doc, css)  # UPLINK PI5
