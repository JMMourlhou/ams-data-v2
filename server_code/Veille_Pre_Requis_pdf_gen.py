...
def veille_pr_requis_pdf_gen(mode="compact", email=None, stage=None):  # stage: stage row
    ...
    if mode == "compact":
        ...
        filename = f"etat_documents_pré_requis_AMS_compact_{date_fichier}.pdf"
    elif mode == "une_page":   # Tous formateurs, 1 page par formateur
        mode_label = "Un formateur par page"
        filename = f"etat_documents_pré_requis_AMS_1page_{date_fichier}.pdf"
    else:
        mode_label = ""   # 1 seul formateur, lancé par recherche
        filename = f"etat_documents_pré_requis_AMS_1_personne_{date_fichier}.pdf"
