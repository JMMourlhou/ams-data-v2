...
import random             # pour envoyer à intervalle différente avec random.randrange(début, fin)
...
def send_mail_randomly(emails_list, subject_txt, rich_text, old_stagiaires=False, attachments=[]):

    # Initialisation du nb de mails envoyés
    nb_mails = 0
    
    # --- Paramètres de délai d'envoi des mails ---
    min_delay = 30                # 30 secondes
    max_delay = 5 * 60          # 5 minutes = 300 secondes
    ...
    for email, prenom, id in emails_list:
        ...
        print(time)
        try:
            # Délai d'envoi d'un mail randomly
            sleep(.01)
...
def run_bg_task_mail_time_sending_randomly(emails_list, subject_txt, rich_text, attachments=[], old_stagiaires=False):
    print("old_stagiaires 1",old_stagiaires)
    task = anvil.server.launch_background_task('send_mail_randomly',emails_list, subject_txt, rich_text, old_stagiaires,attachments)
    return task
