...
def add_stagiaire(stagiaire_row, stage, mode_fi, type_add="", pour_stage=None):   # Stage num pas row
    ...
    if stagiaire_row:
        ...
        # Si le nouveau est un formateur, Admin ou Bureau, ne pas modifier son role
        if user['role'] == "F" or user['role'] == "A" or user['role'] == "B":
            user.update(temp = 0)
        else:
            user.update(temp = 0,
    ...
    if type_stage_row:
        ...
        if dico_pre_requis is not None:   # il y a des clefs pre-requis
